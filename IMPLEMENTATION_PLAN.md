...
- [ ] SSD art pass (ship silhouette behind the boxes, like a real SFB sheet).
	  **See `ART_PLAN.md`** — full asset inventory with authoring specs (sizes,
	  orientation, naming, folder layout) so art can be produced offline during
	  downtime, plus the ordered wiring checklist for hooking assets into
	  `hex_map`/`ssd_panel`/`main_menu` in small sessions.
- [ ] macOS export preset, icon, notarization.
